...
def progress_payload(book,progress):
    ...
    chapters=sorted(book.chapters,key=lambda c:c.sort_order);chapter_ids={c.id for c in chapters}
    if progress.chapter_id not in chapter_ids:return None
    total=sum((c.duration_seconds or 0) for c in chapters);before=0;current=None
    for c in chapters:
        if c.id==progress.chapter_id:current=c;break
        before+=c.duration_seconds or 0
    if not current:return None
    position=max(0,float(progress.position_seconds or 0));stored_pct=max(0,float(progress.progress_percent or 0))
    if position < MIN_PROGRESS_SECONDS and stored_pct < MIN_CHAPTER_PROGRESS_PERCENT:return None
...
def latest_valid_progress(book):
    ...
    return None
def as_detail(book):
    return {**audiobook_item(book),'contained_books':contained_books(book),'latest_progress':latest_valid_progress(book),'chapters':[chapter_item(c) for c in sorted(book.chapters,key=lambda c:c.sort_order)]}
@router.get('/')
...
def update_progress(audiobook_id:int,payload:ProgressUpdate,db:Session=Depends(get_db)):
    ...
    for c in chapters:
        ...
        before+=c.duration_seconds or 0
    position=max(0,float(payload.position_seconds or 0));chapter_pct=max(0,float(payload.progress_percent or 0))
    if position < MIN_PROGRESS_SECONDS and chapter_pct < MIN_CHAPTER_PROGRESS_PERCENT:
        book.status='available';db.commit();return {'status':'ignored','book_status':book.status,'overall_progress_percent':0}
    overall=((before+position)/total*100) if total else chapter_pct
...
def reset_audiobook(audiobook_id:int,db:Session=Depends(get_db)):
    ...
    book.status='available';db.commit();db.refresh(book);return {'book_status':book.status,'progress_deleted':progress_deleted,'latest_progress':None}
