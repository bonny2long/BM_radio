...
export type AudiobookDetail=Audiobook&{contained_books?:ContainedBook[];latest_progress?:{chapter_id?:number;position_seconds:number;progress_percent:number;chapter_progress_percent?:number;overall_progress_percent?:number;completion_state?:string;checkpointed_at?:string;updated_at?:string}|null;chapters:Chapter[]}
export type Station={id?:number;name:string;type:string;seed_value?:string|null;track_count:number;source?:'system'|'user';favorite?:boolean;family?:string|null;display_family?:string|null;featured?:boolean;is_family_station?:boolean}
...
export type ScanRunRecord={id:number;media_kind:'music'|'audiobook'|string;status:'running'|'succeeded'|'failed'|string;started_at?:string|null;completed_at?:string|null;duration_seconds?:number|null;roots:string[];items_discovered:number;items_added:number;items_updated:number;items_unavailable:number;error_count:number;error_summary?:string|null;stale?:boolean;roots_parse_error?:boolean}
export type LibraryIntegrityIssue={id?:string;type:string;category?:string;severity:'info'|'notice'|'warning'|'error';confidence?:'strong'|'possible'|'notice';title:string;message:string;description?:string;count:number;items:Record<string,unknown>[];sample_truncated?:boolean;read_only?:boolean}
export type LibraryIntegritySummary={total_tracks:number;available_tracks?:number;unavailable_tracks?:number;total_albums:number;available_albums?:number;unavailable_only_albums?:number;total_audiobooks:number;available_audiobooks?:number;unavailable_audiobooks?:number;total_audiobook_chapters?:number;available_audiobook_chapters?:number;unavailable_audiobook_chapters?:number;partial_audiobooks?:number;audiobooks_with_unavailable_progress_chapter?:number;successful_scan_runs?:number;failed_scan_runs?:number;running_scan_runs?:number;stale_running_scan_runs?:number;latest_music_scan_status?:string|null;latest_audiobook_scan_status?:string|null;unavailable_track_playlist_memberships?:number;unavailable_track_favorites?:number;unavailable_track_thumbs?:number;unavailable_track_playback_events?:number;duplicate_music_track_release_rows:number;duplicate_music_release_candidates?:number;strong_duplicate_candidates?:number;possible_duplicate_candidates?:number;suspected_duplicate_recordings:number;variant_release_candidates?:number;duplicate_audiobook_editions:number;audiobook_variants?:number;missing_covers:number;unknown_genres:number;weak_titles_corrected:number;weak_title_candidates?:number;scanner_warnings:number;books_indexed?:boolean}
export type LibraryIntegrityReport={generated_at?:string;read_only?:boolean;availability_policy?:string;summary:LibraryIntegritySummary;latest_scans?:Record<string,ScanRunRecord|null>;issues:LibraryIntegrityIssue[]}
export type ScanRunHistoryResponse={items:ScanRunRecord[];limit:number;read_only:boolean}
...
export const updateAudiobookProgress=(id:number,p:{chapter_id:number;position_seconds:number;progress_percent:number;checkpointed_at:string})=>request(`/audiobooks/${id}/progress`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)})
...
export const getLibraryScanRuns=(limit=25)=>request<ScanRunHistoryResponse>(`/library/scan-runs?limit=${limit}`)
