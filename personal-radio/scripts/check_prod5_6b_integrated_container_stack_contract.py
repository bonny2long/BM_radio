...
def main() -> int:
    ...
    check("location ^~ /api/" in nginx and "proxy_pass http://backend:8094" in nginx and all(token in nginx for token in ("location ^~ /__bm_audiobooks/", "internal;", "alias /media/Audiobooks/Library/;")), "9 /api reverse proxy and internal audiobook offload exist")
    check("/api/media" in nginx and "try_files $uri =404" in nginx and "location ^~ /api/" in nginx and "target=/media/Audiobooks/Library,readonly" in build_run, "10 audited media routes cannot fall into SPA fallback and audiobook offload is read-only")
    ...
    check(all(path in build_run for path in ("/media/Music,readonly", "/media/Audiobooks/Library,readonly", "/media/Books,readonly")) and "bm-radio-audiobooks:/media/Audiobooks/Library:ro" in frontend, "19 backend media and frontend audiobook-offload mounts are read-only")
