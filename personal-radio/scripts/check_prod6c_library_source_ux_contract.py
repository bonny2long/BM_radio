...
def main() -> int:
    ...
    audiobook_delivery = all(token in media_routes for token in (
        ...
        "OPEN_ENDED_BYTE_RANGE.fullmatch", "return safe_audiobook_file(cached_metadata, request)",
        "'Content-Range': f'bytes {start}-{end}/{metadata.size}'", "'Accept-Ranges': 'bytes'",
        "cached_audiobook_file_metadata(audiobook_id, chapter_id)", "cache_audiobook_file_metadata(audiobook_id, chapter_id, cached_metadata)",
        "resolve_audiobook_file_metadata", "await run_in_threadpool(resolve_audiobook_file_metadata, audiobook_id, chapter_id)",
        "with SessionLocal() as db", "validated_audiobook_file_metadata(chapter.path)", "with metadata.path.open('rb') as stream",
