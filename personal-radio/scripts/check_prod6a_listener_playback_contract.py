...
def main() -> int:
    ...
    check("generated/regenerable development audio" in live and "FIXTURE_SENTINEL" in live and "archive/NAS media" in live, "3 copied/test-media guard and classification exist")
    ...
    check("full media stream" in live and 'full_status != 200' in live and 'content-length' in live, "9 full stream regression exists")
    ...
    check('"/api/search?q=Acceptance%20Tone%201"' in live and "search_to_play" in live, "22 search-to-play acceptance exists")
    ...
    check("check_prod6a_listener_playback_contract.py" in prod0 and prod0_mandatory_count(prod0) == 59, "40 full PROD0 preserves 59 mandatory checks")
